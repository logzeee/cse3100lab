// Can you see from the example that following points on fork() from the slides
// are valid Cannot assume that they return at the same time or which one
// “returns first”
//  (even on a uni-core)
...
int main(int argc, char *argv[]) {
  ...
  fclose(fp);
  // use wc command to count the number of lines in the result_file and think
  // why we get that number. Note each process writes one line in the file.
  return 0;
}
