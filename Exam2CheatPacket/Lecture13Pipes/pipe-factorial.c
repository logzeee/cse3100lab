...
// Read from a pipe
void read_solution(int pd, int *solution) { read(pd, solution, sizeof(int)); }
...
int main(int argc, char *argv[]) {
  ...
  pid_t pid = fork();
  if (pid == 0) {
    // We close pd[0] since the child process does not need to read from the
    // pipe
    close(pd[0]);
    int n = atoi(argv[1]);
  ...
  printf("result = %d\n", result);
  // We close pd[0] since the parent process no longer needs to read from the
  // pipe
  close(pd[0]);
  waitpid(pid, NULL, 0);
  return 0;
